...
void MainWindow::initGUI() {
    ...
    case RANDOM_GENERATE:
        ...
        ui->comboDifficulty->insertItem(0, QIcon(":/image/GreenDot"), tr("简单"), TableIO::EASY);
        ui->comboDifficulty->insertItem(1, QIcon(":/image/YellowDot"), tr("普通"), TableIO::NORMAL);
        ui->comboDifficulty->insertItem(2, QIcon(":/image/RedDot"), tr("困难"), TableIO::HARD);
        break;
...
void MainWindow::newGame() {
    if(STATUS == PLAYING){
        if (showWarningDialog(tr("开始新游戏将丢失当前进度。确定吗？")) != QMessageBox::Yes) return;
    }
    STATUS = PLAYING;
    setButtonStatus();
    lcdtimer->restart();
...
void MainWindow::difficultyChanged(int index)
{
    if(index == -1) return;
    if(STATUS==PLAYING){
        if(showWarningDialog(tr("切换难度/关卡会开始新游戏，当前游戏进度将会丢失，确定吗？")) != QMessageBox::Yes) return;
    }
...
{
    if(STATUS==PLAYING){
        if(showWarningDialog(tr("切换语言会导致程序重新加载，当前游戏进度将会丢失，确定吗？")) != QMessageBox::Yes) return;
    }
    switch (LANGUAGE) {
    case CHINESE:
        LANGUAGE = ENGLISH;
...
void MainWindow::success() {
    ...
    if(MODE == INTERNAL_PUZZLE){
        //TODO write best time and success flag to database
    }
}
...
{
    return QMessageBox::warning(this, tr("警告"), message,
                                QMessageBox::Yes, QMessageBox::No);
}

void MainWindow::clearRequested()
{
    if(showWarningDialog(tr("确认要清除所有已填写的数字吗？该操作不可撤销，当前进度将会丢失。")) != QMessageBox::Yes) return;
...
{
    if(STATUS==PLAYING){
        if(showWarningDialog(tr("切换模式会导致当前游戏进度丢失，是否确认？")) != QMessageBox::Yes) return;
    }
...
void MainWindow::initializeSignals()
{
    ...
    connect(ui->switchLanguageButton, &QPushButton::clicked, this, &MainWindow::switchLanguage);
